...
public class Madapter_pass_exam extends BaseAdapter {
    //row est une class qui représente l’objet à afficher
    ArrayList<Exam> list=new ArrayList<Exam>();
    Context context;
    public Madapter_pass_exam(ArrayList<Exam> list, Context c){
        this.list=list;
        this.context=c;
    }
    @Override
              ...
              final String n=list.get(ctr).getNum_Exam().toString();
                try{
                   ...
                   // db.delExam(n);
                    ...
                    Toast.makeText(context, "Exam selectionné", Toast.LENGTH_SHORT).show();
                }catch(Exception ex)
                {
